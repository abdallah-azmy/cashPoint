...
import 'package:cashpoint/src/UI/Authentication/ForCashier/sendingCodeForCashier.dart';
...
class ForgetPasswordForCashier extends StatefulWidget {
  @override
  _ForgetPasswordForCashierState createState() => _ForgetPasswordForCashierState();
}

class _ForgetPasswordForCashierState extends State<ForgetPasswordForCashier> {
  String phone ;
  ...
  forgetPassword() async {
    ...
    if (phone == null) {
      ...
      await ApiProvider(_scafold, context)
          .forgetPasswordForCashier(phone: phone )
          .then((value) async {
        if (value.code == 200) {
...
//          userType == 2 ? _prefs.setString('login', "متجر") : _prefs.setString('login', "عميل") ;
          ...
          Future.delayed(Duration(seconds: 1,milliseconds: 250),(){
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => SendingCodeForCashier(phone:phone,typeOfCode:"forgetPassword",)));
