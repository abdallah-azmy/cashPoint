...
import 'package:cashpoint/src/UI/MainScreens/Cashier/OrderDetailsCashier.dart';
...
class MyOrdersCashier extends StatefulWidget {
  final scaffold;
  final openDrawer;
  MyOrdersCashier({this.scaffold, this.openDrawer});
  @override
  _MyOrdersCashierState createState() => _MyOrdersCashierState();
}

class _MyOrdersCashierState extends State<MyOrdersCashier> {
...
//    print("api_token >>>>> $apiToken");
  ...
  getData() async {
    ...
    await ApiProvider(_key, context)
        .getAllOrdersCashier(apiToken: apiToken)
        .then((value) async {
      if (value.code == 200) {
...
//  }
  ...
  refund(id, reason) async {
    LoadingDialog(_key, context).showLoadingDilaog();
    await ApiProvider(_key, context)
        .refundRequestCashier(apiToken: apiToken, transaction_id: id,refund_request_reason: reason)
        .then((value) async {
      if (value.code == 200) {
        Navigator.pop(context);
